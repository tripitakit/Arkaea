...
- **Audit eventi**: 8/14 categorie già emesse e visibili nel ledger (HGT split per canale incluso `conjugation`, `transformation_event`, `transduction_event`, `phage_infection`, `rm_digestion`, `plasmid_displaced`, `bacteriocin_kill`, `error_catastrophe_death`). Restano da chiudere: `sos_active`, `mutator_emergence`, `biofilm_formation`/`_dispersal`, `migration_pulse` in Fase 21 #3; `domain_flip`, `gene_chimera_birth` in Fase 26.
