...
- **Analysis tools**: ✅ trait tracker time-series closed in Phase 21 #4 (trait selector in the Trends tab; 10 scalar/boolean traits sampled every 10 ticks). Still to close: genome diff between lineages (Phase 22 macro / Phase 26 codon-level), metabolic map (Phase 22), violin phenotype distribution (Phase 22), per-gene expression + regulatory network (Phase 25), codon-level viewer + mutation hotspot + kcat×Km landscape (Phase 26).
