...
- **Audit events**: 12/14 categories emitted and visible in the ledger / audit log. Closed in Phase 21: `conjugation` (kind promoted from `hgt_transfer`), `sos_active`, `mutator_emergence`, `biofilm_formation`/`_dispersal`, `migration_pulse`. Already emitted pre-Phase 21: `transformation_event`, `transduction_event`, `phage_infection`, `rm_digestion`, `plasmid_displaced`, `bacteriocin_kill`, `error_catastrophe_death`. Still to close: `domain_flip`, `gene_chimera_birth` in Phase 26 (codon-level events).
...
- **Analysis tools**: ✅ Phase 21 #4 → trait tracker time-series (trait selector in the Trends tab; 11 scalar/boolean traits sampled every 10 ticks). ✅ Phase 22 → metabolic map of the biotope (heatmap 13 metabolites × phases, per-row normalisation), phenotype-distribution strip plot (abundance-weighted centre of mass), macro genome diff (shared vs unique genes/plasmids/prophages + phenotype Δ), biotope-level stress chip in the header (green / amber / red). Still to close: per-gene expression + regulatory network (Phase 25); codon-level viewer + mutation hotspot + kcat×Km landscape + codon diff (Phase 26).
