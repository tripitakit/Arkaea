...
| **L1.7** | R-M (`defense.ex`): opaque 4-codon `signal_key` values represent recognition sites. **No nucleotide sequence model**, no distinction between Type I / II / III. Methylation tracked as a list of keys (no per-nucleotide). | Restriction enzyme specificity is not inspectable at sequence level; methylation is not visualisable as a pattern. | ⚠️ **Partially closed Phase 30** (view-layer only): new `Arkea.Sim.HGT.RecognitionSite` struct exposes the *full codon pattern* (20 codons vs the legacy 4-codon `signal_key`) + classification `:type_i` / `:type_ii` / `:type_iii` (palindrome → Type II EcoRI-like; bipartite asymmetric → Type I EcoKI-like; asymmetric short core → Type III EcoP15I-like). `Phenotype.rm_profiles_detailed/1` produces `[%RecognitionSite{}]` for restriction + methylation activities; `Arkea.Views.RestrictionInspector.build/1` aggregates `type_distribution` + `self_protected`/`unprotected` signatures for the UI consumer. **Runtime defense unchanged**: `Defense.restriction_check/3` still matches on the legacy 4-codon signature → Phase-12 R-M calibration preserved. **Still deferred**: per-position methylation patterns + sequence-level matching mechanism (would require a cross-stack redesign of `signal_key`); the bipartite Type I classifier is a heuristic (zero-codon count ≥ 50% in the middle 50% of the pattern) — a future refinement could introduce a codon-complement involution `c → 19 - c` for non-exact palindromes. |
