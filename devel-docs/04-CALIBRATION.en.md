...
|---|---|---|---|
| **L1.1** | `Gene.promoter_block` and `Gene.regulatory_block` are `nil` in Phase 1 (parsing deferred to generative Phase 3). | Promoters and riboswitches are not inspectable, do not mutate, and do not drive expression. The "regulation" advertised in the manual is reduced to a global scalar. | ⚠️ **Partially closed Phase 25 / 7.3 (parser-only)**: new `Arkea.Genome.Regulation` module with `promoter_sites/1` (4-codon stride → `{signature, strength}`) and `riboswitches/1` (5-codon stride → `{metabolite_id, threshold, mode, magnitude}`). Pure parsing — codon convention documented; partial trailing strides dropped gracefully. **The fields remain `nil` by default** until a builder populates them (`Gene.from_codons/1` does not yet emit them — TODO 7.3-builder). The runtime σ that *acts on* these structures lands in 7.2b. |
...
| **L1.3** | `Gene.operon_id` is a data field (UUID) but **the module `Arkea.Genome.Operon` does not exist**, nor does a coordinated-expression runtime. Genes under the same operon are not co-transcribed. | The operon as a regulatory unit does not function; the manual must not use it as an operational concept. | ⚠️ **Partially closed Phase 25 / 7.2a (structural)**: `Arkea.Genome.Operon` module now exists with `operons/1`, `solo_genes/1`, `containing/2`, `same_operon?/3`, `leader_gene/2` — provides the *structural surface* (grouping by `operon_id` in chromosome order, leader-gene lookup) consumable by view layers. Runtime σ-factor coordination (leader-driven sigma, polycistronic kcat scaling) shipped as 7.2b in the same phase. |
...
- ⚠️ **Per-gene expression (view-only)** — *Closed Phase 25 / 2.2 (view layer)*: `Arkea.Views.GeneExpression.derive(genome, signal_pool)` returns `[%{gene_id, base_level, modulation, expression}]` per chromosome gene. `base_level = 1.0` if the gene carries at least one structural domain, `0.0` otherwise; `modulation = Σ(±cooperativity × binding_affinity)` from co-located `:regulator_output` domains in the same gene (sign by mode), clamped to `[-1.0, 1.0]`; `expression = clamp(base + modulation, 0.0, 2.0)`. Self-modulation only — the cross-gene σ-aware coordination ships in 7.2b. The `kind: "gene_expression"` time-series sample (per-tick persistence) is additive and ships as 2.2-persistence once the Trends-tab consumer is ready.
