...
| **L1.7** | R-M (`defense.ex`): `signal_key` opachi a 4 codoni rappresentano i siti di riconoscimento. **Nessuna sequenza di nucleotidi modello**, nessuna distinzione tra Type I / II / III. Methylation tracciata come lista di chiavi (no per-nucleotide). | La specificità delle restrittasi non è ispezionabile a livello di sequenza; metilazione non visualizzabile come pattern. | ⚠️ **Partially closed Phase 30** (view-layer only): nuovo `Arkea.Sim.HGT.RecognitionSite` struct espone il *pattern codonico completo* (20 codoni invece dei 4 della legacy `signal_key`) + classificazione `:type_i` / `:type_ii` / `:type_iii` (palindromo → Type II EcoRI-like; bipartite asymmetric → Type I EcoKI-like; asymmetric short core → Type III EcoP15I-like). `Phenotype.rm_profiles_detailed/1` produce `[%RecognitionSite{}]` per restriction + methylation; `Arkea.Views.RestrictionInspector.build/1` aggrega type_distribution + self_protected/unprotected signatures per il consumer UI. **Runtime defense unchanged**: `Defense.restriction_check/3` continua a matchare sulla legacy 4-codon signature → calibrazione Phase-12 preservata. **Resta deferred**: per-position methylation patterns + sequence-level matching mechanism (richiederebbe redesign cross-stack di `signal_key`); il classifier bipartite Type I è euristico (count zero-codon ≥ 50% nel middle 50%) e potrà raffinarsi con codon-complement involution `c → 19-c` per palindromi non-esatti. |
