...
| **L1.7** | R-M (`defense.ex`): `signal_key` opachi a 4 codoni rappresentano i siti di riconoscimento. **Nessuna sequenza di nucleotidi modello**, nessuna distinzione tra Type I / II / III. Methylation tracciata come lista di chiavi (no per-nucleotide). | La specificità delle restrittasi non è ispezionabile a livello di sequenza; metilazione non visualizzabile come pattern. | ✅ **Closed Phase 30 (view) + Phase 31 (sequence-level matching) + Phase 32 (refinements)**: Phase 30 ships `RecognitionSite` struct (full 20-codon pattern + Type I/II/III) + `Phenotype.rm_profiles_detailed/1` + `Views.RestrictionInspector`. Phase 31 chiude il runtime: `methylated_positions :: MapSet` per-codon-position; `protected_by?/2` valida coverage (signature + pattern equality + per-position MapSet.subset); `Defense.restriction_check_sequence/3` (+virion variant); `Virion.methylation_sites` + `DnaFragment.methylation_sites` rich profiles; `Phage.run_rm_and_outcome/5` preferisce sequence-level path quando entrambi i lati hanno dati ricchi, fallback legacy 4-codon (preserva calibrazione Phase 12). Phase 32 aggiunge i refinements biologicamente realistici: (a) **codon-complement involution** `c → 19-c` plays the role of Watson-Crick base-pair complement; `palindrome_kind/1` returns `:exact | :complement | :none` e `classify_type/2` ora cattura le Type II reverse-complement palindromiche (textbook EcoRI shape `5'-GAATTC-3'`); (b) **kcat-modulated partial methylation** — `scale_methylation_to_kcat/2` scala `methylated_positions` proporzionalmente al `:catalytic_site.kcat` della metilasi (range 0..10): metilasi degradata → solo le posizioni 5'→3' iniziali metilate → restriction enzymes possono ancora cleavare le posizioni trailing; `Phenotype.rm_profiles_detailed/1` applica lo scaling automaticamente. |
...
**Phase 32 orchestration layer**: ✅ `:scheduled_dosing` closed via `Arkea.Sim.Intervention.Scheduler.schedule/1` + `Arkea.Sim.Intervention.ScheduledDosingWorker` (Oban worker, queue `:scheduled_dosing`). `Scheduler.schedule/1` valida il command e lo accoda con biotope_id + due_tick. Il worker poll-style verifica se il biotope ha raggiunto due_tick: se sì → `BiotopeServer.apply_intervention/2`; se no → `{:snooze, 30s}` (re-tentativo). Cancellazione su biotope spento → `{:cancel, :biotope_not_running}`. v1: single-shot only (no recurrence); tick-based (no wall-clock); cancellazione user-side via `Oban.cancel_job/1`.

**Resta deferred**: heterologous expression as a *single* surgical command — il workflow corrente è `:lineage_inoculation` con un genoma custom (funzionalmente equivalente), ma manca un comando esplicito "inietta questo singolo gene in un lignaggio esistente".
