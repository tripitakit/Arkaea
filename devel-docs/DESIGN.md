...
##### Fase 15 — Xenobiotici e RAS (Resistance Antibiotic Selection)

- **Catalogo xenobiotici** (`Arkea.Sim.Xenobiotic`): mappa atom → entry con `target_class`, `kd`, `mode` (`:cidal | :static | :mutagen`), `degradable_by_hydrolase`. La v1 ha `:beta_lactam` (target `:pbp_like`, Kd 10.0, cidal, degradabile). Catalogo additivo — nuove entries non rompono nulla esistente.
- **Pool xenobiotici** (`Phase.xenobiotic_pool :: %{xeno_id => float}`): concentrazione in scala metabolite_pool. Diluizione in `Phase.dilute/1` come gli altri pool.
- **Tre meccanismi di resistenza emergenti** (tutti generative dai 11 domini esistenti):
  1. **Target absence**: `Phenotype.target_classes :: %{atom => float}` (proxy abundance per `:pbp_like`, `:dna_polymerase_like`, `:ribosome_like`, `:membrane`). PBP-like = co-occorrenza `:transmembrane_anchor + :catalytic_site`. Lineage senza il target → resistenza intrinseca.
  2. **Degradazione enzimatica**: `Phenotype.hydrolase_capacity :: float` = conteggio geni con `:substrate_binding + :catalytic_site(reaction_class: :hydrolysis)` (β-lattamasi-like). `Xenobiotic.degradation_amount/3` = `k_deg × conc × hydrolase × abundance`, capped at conc.
  3. **Efflux pump**: `Phenotype.efflux_capacity :: 0.0..1.0` = co-occorrenza `:transmembrane_anchor + :channel_pore + :energy_coupling + :substrate_binding` capped at 1.0 (max protezione 90%).
- **Modello binding** (`Xenobiotic.bound_fraction/2`): saturating Hill-like `bound = [drug_intracellular] / (Kd + [drug_intracellular])`. Survival per drug = `1 - bound × mode_severity × target_abundance`. Mode severity: cidal 0.95, static 0.50, mutagen 0.0 (Phase 17 cablerà il mutagen a SOS µ).
- **Pipeline** (`Tick.step_xenobiotic`): tra `step_metabolism` e `step_biomass`. (1) per ogni lineage applica `Xenobiotic.survival_factor/3` al suo `atp_yield_by_lineage` post-toxicity. (2) per ogni xenobiotico degradabile, somma la degradazione di tutti i lineage e la sottrae al pool. Skipped quando tutti i pool sono vuoti (no overhead in absence of drugs).
- **Player intervention** (`Arkea.Sim.Intervention :xenobiotic_pulse`): `kind: :xenobiotic_pulse, phase_name:, xenobiotic_id: :beta_lactam, dose: 50.0` inietta una pulse nel pool. Default dose 50.0.
...
##### Fase 18 — Cross-feeding closure, biofilm, Poisson mixing

- **Cross-feeding closure** (`Metabolism.byproducts/1` + `Metabolism.compute_byproducts/1`): tabella stoichiometrica per-substrato che chiude i cicli C/N/S/Fe/H₂. Glucose → acetate + CO₂ + H₂; acetate → CO₂; lactate → acetate + CO₂ + H₂; SO₄²⁻ ⇌ H₂S; NH₃ ⇌ NO₃⁻; CO₂ ⇌ CH₄; CH₄ → CO₂. Coefficienti frazionari (yield per unità di substrato consumato), non strict mass-conservation — l'obiettivo è la chiusura qualitativa dei cicli che permette syntropy emergente (sulfato-riduttore → sulfo-ossidatore, fermentatore → metanogeno idrogenotrofico). Cablato in `Tick.process_phase`: i by-product sono aggiunti al `phase.metabolite_pool` *dopo* il consumo, così il donor non può immediatamente ri-uptake il proprio waste.
- **Biofilm capability** (`Phenotype.biofilm_capable? :: boolean()`): true quando il genoma esprime sia un'ancora superficiale (qualsiasi `:surface_tag`) sia una proteina strutturale multimerica (`:structural_fold` con `multimerization_n ≥ 2`, proxy della matrice EPS). Il doppio requisito previene falsi-positivi: un singolo adesin senza scaffold strutturale non forma biofilm.
- **Biofilm dilution relief** (`Tick.step_environment` Phase 18 update): lineage biofilm-capable ricevono uno sconto `@biofilm_dilution_relief = 0.5` sul rate di dilution della loro fase (50% di residenza extra rispetto ai planctonici alla stessa rate fisica). Sconto per-lineage applicato moltiplicativamente: `effective_rate = base_rate × (1 - 0.5) = 0.5 × base_rate`.
- **Mixing event Poisson** (`Tick.step_mixing_event`): roll Bernoulli per tick contro `@mixing_event_probability = 1.0e-4`. Su hit: redistribuzione uniforme delle abbondanze del lignaggio across phases (analogue di tempesta / sediment turnover / mescolamento intestinale) + omogeneizzazione di metabolite_pool e signal_pool. No-op su biotopi mono-fase. RNG-frugal: il roll consuma un solo `:rand.uniform_s` per tick. Niente actor / payload (è una disturbance naturale), distinto dall'intervention `:mixing_event` player-triggered che resta disponibile via `Arkea.Sim.Intervention`.
- **Pipeline `Tick`**: nuovo `step_mixing_event` tra `step_lysis` e `step_pruning` (mescolamento dopo che le morti sono state contate, prima del cap-enforcement). Sequence finale: `step_metabolism → step_xenobiotic → step_biomass → step_signaling → step_bacteriocin → step_expression → step_cell_events → step_dna_damage → step_hgt → step_phage_infection → step_environment → step_lysis → step_mixing_event → step_pruning`.

##### Fase 19 — Community Mode (sim core)

- **`Lineage.original_seed_id :: binary() | nil`**: tag opzionale che identifica il *founder seed* da cui un lignaggio discende. Propagato automaticamente attraverso `new_child/4` — quindi attraverso ogni evento riproduttivo (mutation, fission, conjugation, transformation, lysogenic / transducing integration). `nil` per residenti pre-Phase-19 o wild biotopes. Non altera la dinamica della simulazione: è puramente una *audit / analytics tag* per ricostruire la genealogia di community-mode senza traversare l'albero dei `parent_id`.
- **`Arkea.Game.SeedLibrary`**: store in-memory player-side delle progettazioni di seed (cap `@max_size = 12` entries). API `new/0`, `save/3` (con check duplicate-name e cap), `delete/2`, `entries/1`, `fetch/2`. La persistenza Ecto-backed (`player_seeds` table) è riservata al PR runtime che cabla la LiveView Seed Lab; il sim core consuma solo il value type `entry()`.
- **`Arkea.Game.CommunityLab.provision_community/3`**: API pura che prende una lista di `SeedLibrary.entry()` e produce una lista di founder lineages indipendenti, ciascuno taggato con `original_seed_id` derivato dall'entry. Vincoli:
  - cap `@max_community_seeds = 3` (anti-deck-building);
  - rifiuta duplicate-id;
  - founders hanno `clade_ref_id` distinti (clades indipendenti).
- **Audit log estensione**: `:lineage_born` e `:hgt_transfer` ora portano `original_seed_id` nel payload. `:community_provisioned` event emesso da `CommunityLab.provision_community/3` con la lista di seed/founder ids — fluisce attraverso il catchall `Atom.to_string` dell'AuditWriter. Cross-clade HGT detection è derivabile post-hoc dai payload `:hgt_transfer` (recipient_seed_id) + lookup donor (parent_id graph) — niente nuovo event type richiesto.
